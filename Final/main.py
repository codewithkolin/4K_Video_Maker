...
from video_processor import create_4k_video, check_ffmpeg_available, estimate_video_file_size, format_file_size, export_overlay_video
...
class VideoCreatorApp:
    def __init__(self, root):
        ...
        self.overlay_output_directory = None
    ...
    def create_widgets(self):
        ...
        # Export Overlay Only button
        self.export_overlay_btn = tk.Button(
            button_frame,
            text="Export Overlay Only",
            command=self.export_overlays_only,
            font=("Arial", 12, "bold"),
            bg="#2196F3",
            fg="white",
            width=20,
        ...
        self.export_overlay_btn.pack(side="left", padx=10)
    ...
    def reset_all(self):
        ...
        # Disable create and export overlay buttons
        self.create_btn.config(state="disabled")
        self.export_overlay_btn.config(state="disabled")
    ...
    def check_ready(self):
        ...
        # Enable Export Overlay Only button if audio files are selected
    ...
    def export_overlays_only(self):
        """Export overlay videos only (without background image)."""
        if len(self.audio_files) == 0:
            messagebox.showwarning(
                "No Audio Files",
        ...
        output_dir = filedialog.askdirectory(
            title="Select Output Directory for Overlay Videos"
        )
        
        if not output_dir:
            return  # User cancelled
        
        self.overlay_output_directory = output_dir
        
        # Disable button during processing
        self.export_overlay_btn.config(state="disabled")
        self.progress_bar['value'] = 0
        self.progress_var.set("Starting overlay export...")
        self.progress_percent_var.set("0%")
        self.status_text.config(state="normal")
        self.status_text.delete(1.0, tk.END)
        self.status_text.config(state="disabled")
        ...
        # Run in separate thread to avoid freezing GUI
        thread = threading.Thread(target=self.process_overlay_export)
        thread.daemon = True
        thread.start()
    
    def process_overlay_export(self):
        """Process overlay export in background thread."""
        try:
            self.log_status("Starting overlay export process...")
            self.update_progress(0, "Initializing...")
            
            # Verify output directory exists
            if not os.path.exists(self.overlay_output_directory):
                os.makedirs(self.overlay_output_directory, exist_ok=True)
            
            # Get audio files info
            audio_files_info = get_audio_files_info(self.audio_files)
            total_files = len(audio_files_info)
            
            self.log_status(f"Exporting {total_files} overlay video(s)...")
            ...
            for i, audio_info in enumerate(audio_files_info):
                ...
                try:
                    from player_overlay_playwright import extract_song_title
                    track_name = extract_song_title(track_path)
                except ImportError:
                    from player_overlay import extract_track_name
                    track_name = extract_track_name(track_path)
                
                # Clean track name for filename (remove invalid characters)
                safe_track_name = "".join(c for c in track_name if c.isalnum() or c in (' ', '-', '_')).strip()
                safe_track_name = safe_track_name.replace(' ', '_')
                
                # Ensure safe_track_name is not empty (fallback to track number if empty)
                if not safe_track_name:
                    safe_track_name = f"track_{i+1}"
                
                # Create output filename (handle duplicates by appending number)
                base_filename = f"{safe_track_name}_overlay.mov"
                output_path = os.path.join(self.overlay_output_directory, base_filename)
                
                # If file exists, append number to make unique
                counter = 1
                while os.path.exists(output_path):
                    name_without_ext = safe_track_name
                    output_path = os.path.join(self.overlay_output_directory, f"{name_without_ext}_overlay_{counter}.mov")
                    counter += 1
                ...
                self.update_progress(
                    file_progress,
                    f"Exporting overlay {i+1}/{total_files}: {track_name}...",
                    int(file_progress)
                )
                self.log_status(f"Exporting overlay {i+1}/{total_files}: {track_name}...")
                
                # Initialize Playwright in worker thread if needed
                try:
                    from player_overlay_playwright import get_browser
                    get_browser()  # Initialize browser in worker thread
                except:
                    pass
                
                # Define progress callback for this overlay
                def overlay_progress_callback(current_time, total_duration):
                    """Update progress for current overlay export."""
                    if total_duration > 0:
                        # Progress within this overlay: 0-100%
                        overlay_progress = (current_time / total_duration) * 100
                        # Overall progress: file_progress + (overlay_progress * file_size_percentage)
                        file_size_percentage = 100 / total_files
                        overall_progress = file_progress + (overlay_progress * file_size_percentage / 100)
                        
                        self.update_progress(
                            overall_progress,
                            f"Exporting overlay {i+1}/{total_files}: {current_time:.1f}s / {total_duration:.1f}s",
                ...
                self.log_status(f"✓ Exported: {output_filename}")
            
            # Close browser after all overlays are exported
            try:
                from player_overlay_playwright import close_browser
                close_browser()
            except:
            ...
            # Final progress
            self.update_progress(100, "All overlays exported successfully!")
            self.log_status(f"\n✓ SUCCESS! Exported {total_files} overlay video(s) to:\n{self.overlay_output_directory}")
            
            def on_success():
                self.progress_var.set("All overlays exported successfully!")
                self.progress_percent_var.set("100%")
                messagebox.showinfo(
                    "Success",
                    f"Exported {total_files} overlay video(s) successfully!\n\n"
                    f"Location:\n{self.overlay_output_directory}\n\n"
                    f"Format: MOV with ProRes 4444 (alpha channel)\n"
                    f"Resolution: 600x174"
                )
                self.export_overlay_btn.config(state="normal")
            self.root.after(0, on_success)
            ...
            def on_error():
                ...
                messagebox.showerror(
                    "Error",
                    f"An error occurred while exporting overlays:\n\n{error_message}"
                )
                # Cleanup temp files on error
                for temp_file in self.temp_files:
                    try:
