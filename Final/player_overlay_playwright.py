...
def create_player_overlay_frame(song_title, current_time, total_time, width=PLAYER_WIDTH, height=PLAYER_HEIGHT, opacity=1.0, slide_offset=0.0):
    ...
    # Calculate font sizes based on height for proportional scaling
    # Track name: 12% of height, Time: 8% of height, Artist: ~7% of height
    title_font_size = max(int(height * 0.12), 12)  # Minimum 12px
    time_font_size = max(int(height * 0.08), 10)    # Minimum 10px
    artist_font_size = max(int(height * 0.065), 9) # Minimum 9px
    ...
    // Scale text sizes proportionally with resolution
    document.getElementById('song-title').style.fontSize = '{title_font_size}px';
    document.getElementById('artist-name').style.fontSize = '{artist_font_size}px';
    document.getElementById('current-time').style.fontSize = '{time_font_size}px';
    document.getElementById('total-time').style.fontSize = '{time_font_size}px';
    """
